...
if (isset($_POST['id'])){
  ...
  $result = $conn->query("SELECT * FROM aboutus_inerr WHERE id='$id' ") or die($conn->error);
    ...
    $_SESSION['image'] =  base64_encode($row['icons']);
...
if(!empty($_FILES["image"]["name"])) { 
    ...
    if( in_array($file_type, $allowed_file_types) ){ 
        ...
        $query = $conn->query("UPDATE aboutus_inerr SET icons = '$img_content' WHERE id = $img_id");
        echo "<script> alert('Icon About us edited.') </script>";
